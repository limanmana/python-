# 单元测试
# 优点：提前暴露问题。缺点；跟业务逻辑代码重复，额外代码量，麻烦。单元测试的逻辑如果错误测试不出来。极限值的考虑不一定正确。所以就建议时间充足的话建议写，时间紧张公司不要求的话做好黑盒测试。
...
# 开始单元测试
import unittest
# assert(断言）  assertEqual(arg1, arg2) 断言相等，如果arg1=arg2正常运行，如果没有满足断言条件 会报错 仍会继续。
